...
function apply(ctx) {
  ...
  const flagsOf = (agent) => {
    ...
    if (!f) { f = { worked: false, errored: false, played: false }; turnFlags.set(agent, f) }
    return f
  }
  ...
  const celebrate = () => {
    ...
    celebrating = true
    setMode('celebrating')
    // 声音改由轮询循环在"每个窗口停止输出"时统一播放，这里不再重复播声
    celebrateTimer = ctx.timeout(() => {
      if (disposed) return
      celebrateTimer = null
      celebrating = false
  ...
  const stopPoll = ctx.interval(() => {
    ...
    // 先跨整表统计本轮仍在输出的窗口，避免遍历顺序影响判断
    const runningNow = new Set()
    for (const a of list) {
      if (a && a.status === 'running') runningNow.add(a)
    }

    for (const a of list) {
      if (!a) continue
      const s = a.status === 'running' ? 'running' : 'idle'
      const prev = lastStatus.get(a)
      lastStatus.set(a, s)
      if (prev === undefined) observedAgents.add(a)
      // 新一轮输出开始时清掉上一轮的临时标记，避免 played/errored 残留
      if (prev === 'idle' && s === 'running') turnFlags.delete(a)

      if (prev === 'running' && s === 'idle') {
        // 单个窗口停止输出
        const f = turnFlags.get(a)
        if (!f || !f.errored) {
          // 每个窗口停止输出都播放一次提示音（不再要求"所有窗口都停"才播）
          // 但如果本回合提问瞬间已经播过，这里不重复播
          if (!f || !f.played) playSystemVoice()
          // 仅当所有窗口都已停止、且无待处理的审批/提问，才进入庆祝动画
          if (runningNow.size === 0 && waitingCount === 0) celebrate()
        }
        if (f) turnFlags.delete(a)
      }
    }
    derive(runningNow.size)
  }, POLL_MS)
  ctx.effect(() => stopPoll)
  ...
  ctx.on('tools/execute', (exec, next) => {
    ...
    let isQ = false
    if (exec && exec.agent) {
      const f = flagsOf(exec.agent)
      f.worked = true
      if (exec.name === 'ask_user_question') {
        // AI 调用提问工具的瞬间 = 停止输出等你回答 → 立即播放提示音
        // 记录 played，避免同一回合停止输出时重复播
        isQ = true
        waitingCount++
        if (!f.played) {
          f.played = true
